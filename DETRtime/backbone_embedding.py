...
# Loading Data
path = f'/segmentation/ICML_zuco_min_segmentation_minseq_500_margin_1_amp_thresh_10000/'
EEG = []
labels = []
for file in os.listdir('../..' + path + 'train'):
    data_train = np.load('../..' + path + 'train/Subject_1_file_0_part_0_segmentation.npz')
    EEG += [data_train['EEG']]
...
EEG = [item for sublist in EEG for item in sublist]
...
# permute samples to (batch, channels, time)
...
while (counts[0] < number) or (counts[1] < number) or (counts[2] < number):
    ...
    emb_time = embeddings[rand1][rand2]
